...
This skill teaches Claude to drive the Autodesk Fusion MCP server safely and efficiently. The MCP exposes three tools that wrap the Fusion Python API:
...
2. Add user parameters first, idempotently. See `patterns.md`, "Idempotent user-parameter add".
...
def run(_context: str):
    ...
    dim_x.parameter.expression = 'length / 2'   # math exprs bind first try
    dim_y.parameter.expression = 'width / 2'
    dim_w.parameter.expression = 'length + 0 mm'   # compound expr binds first try (avoids G6 bare-name freeze)
    dim_d.parameter.expression = 'width + 0 mm'
